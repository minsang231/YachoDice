...
void CRankDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST3, m_listScore); // 점수 랭킹 리스트
	DDX_Control(pDX, IDC_LIST1, m_listWin);   // 승리 랭킹 리스트
}
...
BEGIN_MESSAGE_MAP(CRankDlg, CDialogEx)
	...
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_LIST3, &CRankDlg::OnCustomdrawList)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_LIST1, &CRankDlg::OnCustomdrawList)
END_MESSAGE_MAP()
...
BOOL CRankDlg::OnInitDialog()
{
	...
	// [점수 랭킹]
	m_listScore.InsertColumn(0, _T("순위"), LVCFMT_CENTER, 80); // 너비 조금 늘림
	m_listScore.InsertColumn(1, _T("ID"), LVCFMT_CENTER, 100);
	m_listScore.InsertColumn(2, _T("최고 점수"), LVCFMT_CENTER, 100);

	// [승리 랭킹]
	m_listWin.InsertColumn(0, _T("순위"), LVCFMT_CENTER, 80); // 너비 조금 늘림
	m_listWin.InsertColumn(1, _T("ID"), LVCFMT_CENTER, 100);
	m_listWin.InsertColumn(2, _T("승리 횟수"), LVCFMT_CENTER, 100);
...
void CRankDlg::LoadRankings()
{
	...
	try {
		...
		// AfxMessageBox(_T("랭킹 DB 연결 실패: ") + e->m_strError);
		e->Delete();
		return;
	}
	...
	try {
		...
		while (!rs.IsEOF())
		{
			...
			// ★★★ [수정] 1, 2, 3등에 이모지 붙이기 ★★★
			if (nRank == 1) strRank = _T("🥇 1위");
			else if (nRank == 2) strRank = _T("🥈 2위");
			else if (nRank == 3) strRank = _T("🥉 3위");
			else strRank.Format(_T("%d위"), nRank);
