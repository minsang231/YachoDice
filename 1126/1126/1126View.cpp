...
void CMy1126View::OnInitialUpdate()
{
	...
	// 컬럼 비율 설정
	float nCol1, nCol2, nCol3;

	if (m_nGameMode == 1) // 1인용
	{
		nCol1 = nTotalWidth * 0.6f;
		nCol2 = nTotalWidth - nCol1;
		nCol3 = 0.f;
	}
	else // 2인용
	{
		nCol1 = nTotalWidth * 0.4f;
		nCol2 = nTotalWidth * 0.3f;
	...
	// ★★★ [수정] 리소스 폴더 경로 설정 (상대 경로 사용) ★★★
	// Debug → x64 → 1126(루트) → 1126(소스) → res
	CString strResPath;
	strResPath.Format(_T("%s\\..\\..\\1126\\res\\"), (LPCTSTR)strExePath);

	// 경로 정규화 (..을 실제 경로로 변환)
	TCHAR szFullPath[MAX_PATH];
	PathCanonicalize(szFullPath, strResPath);
	strResPath = szFullPath;
...
void CMy1126View::DrawGame()
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		// ★★★ [수정] 주사위 그리기 (애니메이션 중이든 아니든 항상 그림) ★★★
